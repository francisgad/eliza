...
// Define our own types since some Citrex SDK types are not exported or have issues
type OrderType = 'LIMIT' | 'MARKET' | 'STOP_LIMIT' | 'STOP_MARKET';
type TimeInForce = 'GTC' | 'IOC' | 'FOK';

/**
 * Interface for order arguments (our own version)
 */
interface OrderArgs {
  isBuy: boolean;
  orderType: OrderType;
...
export class SeiPerpetualTradingService extends Service {
  ...
  async getOpenOrders(productSymbol?: string): Promise<Order[]> {
    try {
      ...
      // Map Citrex SDK orders to our simplified interface
  ...
  async placeOrder(orderArgs: OrderArgs): Promise<any> {
    try {
      ...
      const orderId = result.order?.id || result.order?.orderId || 'N/A';

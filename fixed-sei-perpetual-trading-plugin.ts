...
import { z } from 'zod';
// Import Citrex SDK at the top level
import CitrexSDK from 'citrex-sdk';
import type { 
  Config as CitrexSDKConfig, 
  OrderArgs as CitrexOrderArgs, 
...
/**
 * Interface for order arguments (mapped to Citrex SDK types)
 */
interface OrderArgs {
  isBuy: boolean;
  orderType: OrderType;
...
/**
 * Interface for product information (mapped from Citrex SDK)
 */
interface Product {
  active: boolean;
  baseAsset: string;
...
/**
 * Interface for position data (mapped from Citrex SDK)
 */
interface Position {
  productId: number;
  symbol: string;
...
/**
 * Interface for order data (mapped from Citrex SDK)
 */
interface Order {
  id: string;
  productId: number;
...
export class SeiPerpetualTradingService extends Service {
  ...
  private citrexConfig: CitrexSDKConfig;
  ...
  constructor(runtime?: IAgentRuntime) {
    ...
    // Create properly typed Citrex config
    this.citrexConfig = {
   ...
   */
  private initCitrexClient(): CitrexSDK {
    try {
      return new CitrexSDK(this.privateKey as `0x${string}`, this.citrexConfig);
    } catch (error) {
  ...
  async getProducts(): Promise<Product[]> {
    try {
      const client = this.initCitrexClient();
      const result: ProductsReturnType = await client.getProducts();
  ...
  async getProduct(productIdOrSymbol: number | string): Promise<Product> {
    try {
      const client = this.initCitrexClient();
      const result: ProductReturnType = await client.getProduct(productIdOrSymbol);
  ...
  async getBalances(): Promise<any> {
    try {
      const client = this.initCitrexClient();
      const result: BalancesReturnType = await client.listBalances();
      return result;
    } catch (error) {
  ...
  async getPositions(productSymbol?: string): Promise<Position[]> {
    try {
      const client = this.initCitrexClient();
      const symbolParam = productSymbol as ProductSymbol | undefined;
      const result: PositionsReturnType = await client.listPositions(symbolParam);
      
      // Map Citrex SDK positions to our interface
  ...
  async getOpenOrders(productSymbol?: string): Promise<Order[]> {
    try {
      const client = this.initCitrexClient();
      const symbolParam = productSymbol as ProductSymbol | undefined;
      const result: OrdersReturnType = await client.listOpenOrders(symbolParam);
      
      // Map Citrex SDK orders to our interface
  ...
  async placeOrder(orderArgs: OrderArgs): Promise<any> {
    try {
      ...
      const client = this.initCitrexClient();
      // Convert our OrderArgs to Citrex SDK OrderArgs
      const citrexOrderArgs: CitrexOrderArgs = {
        ...
        stopPrice: orderArgs.stopPrice,
        clientId: orderArgs.clientId,
      };
      
      const result: PlaceOrderReturnType = await client.placeOrder(citrexOrderArgs);
      
      logger.info(`Order placed successfully: ${result.order?.id || 'N/A'}`);
      return result;
    } catch (error) {
  ...
  async placeOrders(orders: OrderArgs[]): Promise<any> {
    try {
      ...
      const client = this.initCitrexClient();
      // Convert our OrderArgs array to Citrex SDK OrderArgs array
      const citrexOrders: CitrexOrderArgs[] = orders.map(order => ({
        isBuy: order.isBuy,
  ...
  async cancelAllOrders(productSymbol: string): Promise<any> {
    try {
      ...
      const client = this.initCitrexClient();
      const result = await client.cancelOpenOrdersForProduct(productSymbol as ProductSymbol);
  ...
  async getOrderBook(productSymbol: string): Promise<any> {
    try {
      const client = this.initCitrexClient();
      const result: OrderBookReturnType = await client.getOrderBook(productSymbol as ProductSymbol);
      return result;
    } catch (error) {
  ...
  async getAccountHealth(): Promise<any> {
    try {
      const client = this.initCitrexClient();
      const result: AccountHealthReturnType = await client.getAccountHealth();
      return result;
    } catch (error) {
